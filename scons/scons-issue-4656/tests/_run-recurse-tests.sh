...
TEST_ROOT="${SCRIPT_ROOT}/RecurseTest"
...
python "${SCONS_ROOT}/scripts/scons.py" --site-dir="${SCONS_SITEDIR}" -Qn --scanner-deps --scanner=${_SCONS_SCANNER} --expected-map="{'main.c': ['include/recurse1.h']}" --sconstruct=SConstruct-NoRecurse
echo --- scons:deps:norecurse:end ---
echo
echo --- scons:deps:beg ---
python "${SCONS_ROOT}/scripts/scons.py" --site-dir="${SCONS_SITEDIR}" -Qn --scanner-deps --scanner=${_SCONS_SCANNER} --expected-map="{'main.c': ['include/recurse1.h']}"
echo --- scons:deps:end ---
echo
