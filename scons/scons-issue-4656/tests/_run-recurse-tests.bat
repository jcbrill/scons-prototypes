...
@set "TEST_ROOT=%SCRIPT_ROOT%\RecurseTest"
...
@echo.
@echo --- scons:deps:norecurse:beg ---
@"%PYEXE%" "%SCONS_ROOT%\scripts\scons.py" --site-dir="%SCONS_SITEDIR%" -Qn --scanner-deps %_SCONS_MSVC_COMPILER% --scanner=%_SCONS_SCANNER% --expected-map="{'main.c': ['include/recurse1.h']}" --sconstruct=SConstruct-NoRecurse
@echo --- scons:deps:norecurse:end ---
@echo.
@echo --- scons:deps:beg ---
@"%PYEXE%" "%SCONS_ROOT%\scripts\scons.py" --site-dir="%SCONS_SITEDIR%" -Qn --scanner-deps %_SCONS_MSVC_COMPILER% --scanner=%_SCONS_SCANNER% --expected-map="{'main.c': ['include/recurse1.h']}"
@echo --- scons:deps:end ---
@echo.
